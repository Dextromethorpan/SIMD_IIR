...
int main(int argc, char** argv){
    ...
    for (int N : {2, 10}) {
        ...
        for (int S : sample_sizes) {
            ...
            for (int t=0; t<args.trials; ++t){
                ...
                double nspt = (dt / S) * 1e9 / std::max(1, N);
    ...
    std::fclose(f);
    std::cerr << "Wrote bench_results_cache.csv\n";
    return 0;
}
